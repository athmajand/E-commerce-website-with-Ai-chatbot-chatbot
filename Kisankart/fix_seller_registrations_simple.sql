-- Fix seller_registrations table script (simplified version)
-- This script will:
...
-- If not, add it (using a safer approach)
SELECT COUNT(*) INTO @email_has_unique
...
SELECT IF(@email_has_unique = 0,
    'ALTER TABLE seller_registrations ADD UNIQUE INDEX email_unique (email)',
    'SELECT "Email already has a unique constraint"') INTO @email_sql;

-- Execute the SQL (will either add the constraint or do nothing)
PREPARE email_stmt FROM @email_sql;
EXECUTE email_stmt;
DEALLOCATE PREPARE email_stmt;

-- Check if phone column has a unique constraint
-- If not, add it (using a safer approach)
SELECT COUNT(*) INTO @phone_has_unique
FROM INFORMATION_SCHEMA.STATISTICS
WHERE table_schema = 'kisan_kart'
AND table_name = 'seller_registrations'
...
-- Only add the constraint if it doesn't exist
SELECT IF(@phone_has_unique = 0,
    'ALTER TABLE seller_registrations ADD UNIQUE INDEX phone_unique (phone)',
    'SELECT "Phone already has a unique constraint"') INTO @phone_sql;

-- Execute the SQL (will either add the constraint or do nothing)
PREPARE phone_stmt FROM @phone_sql;
EXECUTE phone_stmt;
DEALLOCATE PREPARE phone_stmt;
