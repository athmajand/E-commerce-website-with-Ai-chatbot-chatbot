...
// Get recent orders (5)
$recent_orders_query = "SELECT o.* FROM orders o
...
if ($recent_orders_result) {
    while ($row = $recent_orders_result->fetch_assoc()) {
        ...
        if (!empty($row['customer_id'])) {
            $customer_query = "SELECT * FROM customer_registrations WHERE id = " . $row['customer_id'];
            $customer_result = $conn->query($customer_query);
            if ($customer_result && $customer_result->num_rows > 0) {
                $customer = $customer_result->fetch_assoc();
                // Use first_name and last_name fields to create customer name
...
<body>
    ...
    <div class="sidebar">
        <div class="sidebar-header">
            <h3><img src="images/farmer-logo.png" alt="Kisan Kart Logo" style="height: 28px; width: 28px; margin-right: 8px;">Kisan Kart Admin</h3>
        </div>
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-4 col-xl-3">
                <div class="card stats-card">
                    ...
                    <h2><?php echo $stats['total_sellers']; ?></h2>
                    <p>Total Sellers</p>
                </div>
            </div>
            <div class="col-md-4 col-xl-3">
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($recent_orders as $order): ?>
                                            <tr>
                                                ...
                                                <td>₹<?php echo number_format($order['total_amount'] ?? $order['amount'] ?? $order['total'] ?? 0, 2); ?></td>
                                                <td>
                                                    <?php
                                                ...
                                                <td>
                                                    <?php
                                                        $date = $order['order_date'] ?? $order['date'] ?? date('Y-m-d H:i:s');
