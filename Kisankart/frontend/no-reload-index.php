...
<head>
    ...
    <script>
    // Store a flag in sessionStorage to prevent auto-refresh loops
    if (sessionStorage.getItem('pageLoaded')) {
        // This is a refresh, not an initial load
    ...
    // Clear the flag when the user navigates away
    window.addEventListener('beforeunload', function() {
        // Only clear if the user is navigating to a different page, not refreshing
...
<body>
    ...
    const products = [
        ...
        },
        {
            ...
            description: "Premium quality potatoes grown without pesticides....",
            price: 30.00,
            image: "https://via.placeholder.com/300x200?text=No+Image"
        },
        {
        ...
        },
        {
            id: 8,
            name: "Organic Apples",
            description: "Sweet and juicy apples from organic orchards....",
            price: 60.00,
            discountPrice: 55.00,
            image: "https://via.placeholder.com/300x200?text=No+Image"
        },
        {
            ...
            price: 33.00,
            discountPrice: 22.00,
            image: "../uploads/products/1747572940_Carrot1.jpg"
        }
    ];
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ${product.discountPrice ?
                                            `<span class="text-decoration-line-through text-muted small">₹${product.price.toFixed(2)}</span><br>
                                            <span class="fs-5 fw-bold text-success">₹${product.discountPrice.toFixed(2)}</span>` :
                                            `<span class="fs-5 fw-bold text-success">₹${product.price.toFixed(2)}</span>`}
                                    </div>
                                </div>
                            </div>
                        </a>
    ...
    // Load products only once when the page loads
    document.addEventListener('DOMContentLoaded', function() {
        // Only display products if they haven't been loaded yet
        if (!document.getElementById('featured-products').hasChildNodes()) {
            displayProducts();
        }
    });
    </script>
